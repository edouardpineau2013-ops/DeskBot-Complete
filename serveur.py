...
from etat_deskbot import (
    obtenir_etat,
    definir_etat,
    definir_reponse
)

from actions.cours import importer_cours, slugifier
from cerveau import traiter_commande

from actions.chronometre import chronometre
from actions.minuteur import minuteur
from actions.alarme import (
    obtenir_alarme_principale,
    prochaine_alarme
)
...
from actions.profil_revision import (
    charger_profil,
    ouvrir_boite_mystere,
    calculer_rarete_profs
)

from actions.revision import (
    generer_questions,
    session_revision
)

from actions.taches import (
    ajouter_tache,
    obtenir_taches
)
...
# Render définit cette variable automatiquement.
EST_RENDER = os.environ.get("RENDER") == "true"
...
if not EST_RENDER:
    try:
        ...
        print("⚠️ Impossible de lancer le gestionnaire mails :", e)
...
def afficher_alarme():
    ...
    if (
        alarme is None
        or "heure" not in alarme
        or "minute" not in alarme
    ):
        return jsonify({
            "existe": False,
            "heure": None,
...
def afficher_mails():
    ...
    total, details = etat_mails_non_lus()
...
def importer_cours_route():
    ...
    matiere = request.form.get(
        "matiere",
        ""
    ).strip()

    chapitre = request.form.get(
        "chapitre",
        ""
    ).strip()
    ...
    if not matiere or not chapitre or fichier is None:
        return jsonify({
            "erreur": "Matière, chapitre et fichier requis"
        }), 400
...
def afficher_profil_revision():
    ...
    return jsonify({
        ...
        "record_serie": profil.get(
            "record_serie",
            0
        ),
        "derniere_revision": profil.get(
            "derniere_revision"
        ),
...
def revision_demarrer():
    ...
    question = (
        session_revision.question_suivante()
    )
...
def revision_repondre():
    ...
    correcte, explication = (
        session_revision.repondre(
            reponse
        )
    )
...
def ajouter_note_route():
    ...
    if not titre:
        ...
        return jsonify({
            "succes": False,
            "erreur": (
                "Le titre de la note est requis."
            )
        }), 400
...
def analyser_image_route():
    ...
    mime_type = extensions_autorisees[
        extension
    ]
